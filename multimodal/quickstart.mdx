...
--- ## 1. Start a therapy session ```bash
...
``` ## 2. Analyze a recorded session ```bash
...
``` The response is the canonical payload — flat `ClinicalMetric` envelopes for the frontend plus the full neurobehavioral profile. ## 3. Audio-only ```bash
