...
--- `wss://api.medera.info/sessions` is the **stateful, diarized live-capture WebSocket** for therapy sessions. Same engine, same calibration, same structured output as `/transcribe` and `/transcripts` — but architected for the full 50-minute encounter with 2-speaker diarization, in-line C-SSRS markers, and structured instrument-score emission. ## Connection ```
wss://api.medera.info/sessions?session_id=<id>&token=<jwt>
``` ## Configuration (client → server) ```json
...
``` ## Events (server → client) | Event | Schema |
| :--- | :--- |
...
| `error` | `{ code, message, recoverable }` | ## Lifecycle ```
...
``` ## Audio - 16 kHz 16-bit PCM mono recommended
- Opus at 32 kbps for bandwidth-constrained edges
- 20 ms frames (320 samples at 16 kHz)
- See [Audio Configuration](/multimodal/stt/audio) for full formats ## What you get back at session close ```json
...
``` <Warning> **C-SSRS crisis markers are non-bypassable.** When `crisis.detected` fires, the practice escalation tree is invoked immediately. The agent cannot gate, suppress, or delay the event.
</Warning> --- ## What's next <CardGroup cols={2}> <Card title="/transcribe" icon="microphone" href="/multimodal/stt/transcribe">Stateless between-visit dictation.</Card> <Card title="/transcripts" icon="file-audio" href="/multimodal/stt/transcripts">Async batch processing.</Card> <Card title="Diarization" icon="users" href="/multimodal/stt/diarization">2-speaker separation.</Card> <Card title="Audio configuration" icon="settings" href="/multimodal/stt/audio">Formats and frame size.</Card>
</CardGroup>