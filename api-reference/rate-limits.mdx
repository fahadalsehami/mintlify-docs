...
Enterprise tenants can request elevated limits.
...
The `requireActiveSubscription` middleware blocks agent / intake / therapy / copilot endpoints when the Stripe subscription is not active. `checkAgentLimit` enforces per-tier agent count.
